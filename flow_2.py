...
class BayesridgeFlow(FlowSpec):
    ...
    N_iter_range=Parameter(
        name='n_iter',
        help='set hypterparameters',
        type=JSONType,
    ...
    def hyperparameters(self):
       self.N_iter=[i for i in self.N_iter_range]
    ...
    def validation(self,inputs):
        ...
        exp.set_name("bayesridge_results")
    ...
    def test(self):
        ...
        self.mse = metrics.mean_squared_error(self.y_test, self.y_test_predicted)
        self.r2 = metrics.r2_score(self.y_test, self.y_test_predicted)
        exp = Experiment(project_name=os.environ['MY_PROJECT_NAME'],
                         # workspace="trancedu",
                         auto_param_logging=False)
        metrics={ "mse" :self.mse,
        ...
        print('The best model based on test set has MSE : {}, R2 score : {}'.format(self.mse,self.r2))
